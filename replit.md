...
- **WhatsApp Integration:** Uses Z-API for various message types, logs interactions, and provides a "Message Center" interface with real-time updates. Supports full media processing, including optimized audio transcription (Whisper) and image/document analysis. Includes message debounce (6s `asyncio` timer per phone to accumulate rapid messages), persistent conversation history loaded from `whatsapp_messages` table (survives deploys), and hierarchical memory: session active (last 10 messages), session summary (GPT-4o-mini 2-3 sentence summary stored in `conversations.last_session_summary`), full history in DB. Query Rewriter integrated in RAG search pipeline for pronoun resolution before vector search. **Pipeline equalizado com Testar Agente:** ambos usam EnhancedSearch, normalize_message, skip RAG para SAUDACAO/ATENDIMENTO_HUMANO/FORA_ESCOPO, contexto de materiais PDF, RetrievalLog, e resumo de sessão.
