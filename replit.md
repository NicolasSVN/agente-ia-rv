...
Toda interação com o assessor na ponta final deve ser processada pelo OpenAI com contexto da conversa para compreensão natural da intenção. O OpenAI é o "cérebro" do Stevan — ele analisa, entende e decide. O sistema fornece "ferramentas" (enviar diagrama, enviar material/PDF, buscar na web, buscar no RAG, escalar para humano) que o OpenAI pode acionar. Regras fixas pré-OpenAI existem apenas como otimização de custo/velocidade para casos triviais, nunca como substituto da compreensão natural. Quando o OpenAI identifica uma ação (ex: enviar diagrama ou material), ele retorna uma marcação especial (ex: `[ENVIAR_DIAGRAMA:slug]` ou `[ENVIAR_MATERIAL:material_id]`) e o sistema executa a ação real.
...
- **Busca Semântica (RAG V3.1 Aprimorado):** Utiliza **pgvector (PostgreSQL)** e OpenAI text-embedding-3-large (3072 dimensões) com ranking híbrido (vetor 70%, recência 20%, match_exato 10%, topic_match). Migrado de ChromaDB para pgvector em fev/2026 para persistência permanente entre deploys. Tabela `document_embeddings` com busca por similaridade coseno. Re-indexação automática na inicialização verifica blocos aprovados sem embedding e indexa automaticamente. Chunks incluem contexto global e metadados semânticos (topic, concepts). Detecção inteligente de ticker com variações (ex: "mana 11" → MANA11). Glossário financeiro com 824 conceitos em 13 categorias (`services/financial_concepts.py`) expande queries automaticamente com termos relacionados. Categorias incluem: ESTRUTURA_FUNDO, PERFORMANCE, DISTRIBUICAO, COMPOSICAO, RISCO, MERCADO, DERIVATIVOS, OPERACIONAL, OPCOES_DERIVATIVOS, RENDA_FIXA, TRADING, ASSESSOR_JARGAO, GLOSSARIO_B3 (677 termos oficiais extraídos do glossário da B3/Bora Investir via script `scripts/extract_b3_glossary.py`). Enriquecimento semântico de chunks via GPT-4o-mini (`scripts/enrich_chunks.py`). Script de validação em `scripts/validate_glossary.py` verifica integridade (parênteses malformados, IDs duplicados, colisões de abreviações). Sistema de avaliação RAG (Nível 2) em `tests/rag_evaluation/` e testes de conversa em `tests/conversation_tests/`.
...
- **pgvector:** Extensão PostgreSQL para armazenamento e busca vetorial (substituiu ChromaDB em fev/2026).
