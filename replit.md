...
- **Campaign Structures with Temporal Validity:** `CampaignStructure` model (`campaign_structures` table) stores active derivative campaigns with ticker, structure_type, campaign_slug, key_data (JSON with strikes/costs/expiry), diagram_filename, and valid_from/valid_until for temporal filtering. Active campaigns are auto-injected into Stevan's system prompt V2 so GPT knows about current campaigns. Campaign-specific diagrams override generic derivatives_dataset diagrams when sending via `send_payoff_diagram`. Campaign dispatch messages are persisted in `whatsapp_messages` so GPT has context when assessors respond later. API endpoints: `/api/campaigns/structures` (CRUD), `/api/campaigns/active-structures`, `/api/campaigns/derivative-slugs`, `/api/campaigns/structures/{id}/diagram`. Frontend management page at `/estruturas-campanha` (`frontend/templates/estruturas_campanha.html`) with CRUD interface, filter by status, key_data editor, and diagram upload. Sidebar link under Campanhas. IMPORTANT: Structure routes in `campaigns.py` are declared BEFORE the generic `/{campaign_id}` catch-all to avoid route conflicts.
